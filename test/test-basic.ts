...
test('prevent sending messages if socket has ended', async function(t) {
    let server = await listeningServer()
    t.pass('Listening')
    let fingerprint = await server.fingerprint()
    t.ok(fingerprint, 'Server shall have a fingerprint')
    ...
    await closeServer(server)
    t.pass('closed')
    t.equal(clientHandler.errors, 0, 'Client handler received no errors')
    t.equal(
        serverClientHandler.messages.length,
        1,
        'Server received one message'
    )
    t.equal(
        serverClientHandler.messages[0],
        'test1',
...
test('prevent sending messages if socket is destroyed', async function(t) {
    let server = await listeningServer()
    t.pass('Listening')
    let fingerprint = await server.fingerprint()
    t.ok(fingerprint, 'Server shall have a fingerprint')
